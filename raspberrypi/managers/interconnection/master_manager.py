...
class MasterManager(TCPTalks, Thread):
    ...
    def _lock_balance(self, *args, **kwargs):
        if not self.Balance.is_set():
            ...
            self.logger("MASTER MANAGER : ",
                        "[{}] Lock balance".format(args[0]))
            self.Balance.set()
            return True
        else:
            self.logger("MASTER MANAGER : ",
                        "Balance already lock by {}".format(self.locker_name))
            return False

    def _unlock_balance(self, *args, **kwargs):
        if self.Balance.is_set() and (self.locker_name == args[0]):
            self.logger("MASTER MANAGER : ",
                        "[{}] UnLock balance".format(args[0]))
            self.Balance.clear()
            return True
        else:
            self.logger("MASTER MANAGER : ", "Cannot UnLock : Balance already lock by {}".format(
                self.locker_name))
            return False
